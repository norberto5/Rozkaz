...
namespace WebApp_OpenIDConnect_DotNet.Services.GraphOperations
{
    public class GraphApiOperationService : IGraphApiOperations
    {
        ...
        {
            httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(Constants.BearerAuthorizationScheme, accessToken);
            var response = await httpClient.GetAsync($"{webOptions.GraphApiUrl}/me");
        ...
        public async Task<string> GetPhotoAsBase64Async(string accessToken)
        {
            ...
            var response = await httpClient.GetAsync($"{webOptions.GraphApiUrl}/me/photo/$value");
            if (response.StatusCode == HttpStatusCode.OK)
            {
